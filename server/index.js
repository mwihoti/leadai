...
const PORT = Number(process.env.TELEGRAM_PORT || process.env.PORT || 8787);
...
async function route(req, res) {
  ...
  try {
    if (req.method === 'GET' && url.pathname === '/api/health') {
      return json(res, 200, { ok: true, botConfigured: Boolean(TOKEN), bot: state.bot });
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/telegram/sync') {
      ...
      await processDueForUser(user);
      await saveStore();
      return json(res, 200, { ok: true, connected: Boolean(user.telegram.chatId), doneTaskIds: user.commandDoneTaskIds || [] });
    }
